...
export class IptvComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get('http://localhost:3000/get_iptv_categories').subscribe((data: any) => {
  ...
  onSubCategorySelected(subCategory: string) {
    ...
    this.http.get(`http://localhost:3000/get_chanel_by_category?category=${encodedSubCategory}`).subscribe((data: any) => {
  ...
  playChannel(channel: any) {
    if (channel.channels.length > 1) {
      ...
      this.http.get(`http://localhost:3000/play_channel_by_url?url=${channel.channels[0].url}`).subscribe((data: any) => {
        if (data.message == '\nExiting... (End of file)\n'){
